...
export async function crearPedidoRemoto(input: {
  ...
  const { notifyPedidoCreado } = await import("./mail");
  const policy = await getCheckoutPolicy();
  if (policy.requiereCuenta && !input.cuentaClienteId) {
    return {
      ok: false,
      error: "Inicia sesión o crea una cuenta para pedir.",
    };
  }
  ...
  if (!input.cuentaClienteId && !emailNorm) {
    return {
      ok: false,
      error: "Indica un email (o inicia sesión) para confirmar tu pedido.",
    };
  }
  ...
  await sqlTransaction(async () => {
    ...
    await sqlRun(
      `INSERT INTO pedidos (
        ...
        ficha_codigo, creado_en, actualizado_en, cuenta_cliente_id, cliente_email
  ...
  void notifyPedidoCreado(pedido, emailNorm);
...
export async function marcarPago(
  ...
  if (pedido && estadoPago === "pagado") {
    ...
    const { getClienteEmailForPedido } = await import("./cliente-auth");
    const { notifyPagoConfirmado } = await import("./mail");
    const email = await getClienteEmailForPedido(pedidoId);
    void notifyPagoConfirmado(pedido, email);
  }
  return pedido;
}
